...
<script type="text/javascript">
    ...
    $(".emp_edit_btn").click(function () {
        ...
        var updateEmpEmail = $(this).parent().parent().find("td:eq(2)").text();
        var updateEmpGender = $(this).parent().parent().find("td:eq(3)").text()=="男" ?"M":"F";
        var updateEmpDeptName = $(this).parent().parent().find("td:eq(4)").text();


        //1.员工信息回显
        $(".emp_update_btn").attr("updateEmpId", updateEmpId);
        $("#update_static_empName").text(updateEmpName);
        $("#update_empEmail").val(updateEmpEmail);
        console.log(updateEmpGender);
        $(".update_emp_form input[name=gender]").val([updateEmpGender]);
        console.log($(".update_emp_form input[name=gender]:checked").val());

        //2 根据id或name查询出对应员工信息进行回显；
        // $.ajax({
        //     url:"/hrms/emp/getEmp/"+updateEmpId,
        //     type:"GET",
